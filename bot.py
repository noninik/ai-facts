...
def call_groq(system, prompt, max_tokens=300):
    headers = {
...
def generate_quote(topic):
    return call_groq(
        ...
        "Напиши один удивительный факт на тему: " + topic + ". Одно предложение. Максимум 15 слов. Без кавычек."
    )


def generate_voice_text(topic):
    return call_groq(
        "Ты ведущий научно-популярного подкаста. Говоришь увлекательно, с интригой, живым языком на русском. Как будто рассказываешь другу невероятную историю.",
        "Запиши голосовое сообщение с удивительным фактом на тему: " + topic + ". "
        "Начни с приветствия и интриги, например: 'Привет! Сейчас я расскажу тебе кое-что невероятное.' "
        "Потом расскажи один удивительный факт с подробностями и цифрами. "
        "Потом объясни почему это важно или удивительно. "
        "В конце скажи: 'Подписывайся чтобы узнавать такое каждый день!' "
        "Всего 5-7 предложений. 60-80 слов. Без кавычек. Без эмодзи.",
        max_tokens=200
    )
